...
function addToList(media, user) {
  ...
  return List.findOne({ _id: new ObjectId(media.list) })
    .then(l => {
      ...
      list = l;
      return Media.findOneAndUpdate({ media_id: media.id }, {$set: {
      ...
      const mediaList = list.media.filter(el => el.item_id.toString() !== m._id.toString()).concat([{ item_id: new ObjectId(m._id), isWatched: false }]);
      return List.findOneAndUpdate({ _id: new ObjectId(media.list ) }, { $set: { media: mediaList } }, { new: true });
    })
    .catch(e => {
      console.error(e);
      return null;
...
function getMediaList(mediaIds) {
  return Media.find({ _id: { $in: mediaIds.map(el => new ObjectId(el.item_id)) } }) // find all media whose _id equals the item_id (MLab id) of the item
  .then(ms => {
    return ms.map(m => ({
      isWatched: mediaIds.find(el => el.item_id.toString() === m._id.toString()).isWatched,
      title: m.title,
      media_id: m.media_id,
...
function removeFromList({ id, list }, user) { // id passed in is the media id from TMDB
  let listData = {};
  return List.findOne({ _id: new ObjectId(list) })
    ...
    .then(ms => {
      ...
      const childItems = ms.filter(m =>
        (m.parent_show && m.parent_show.toString() === item.id.toString()) ||
        (m.parent_season && m.parent_season.toString() === item.id.toString())
      ).map(el => el.id.toString()).concat([item.id.toString()]);
      return List.findOneAndUpdate({ _id: new ObjectId(list) },
        { $set: { media: listData.media.filter(el => !childItems.includes(el.item_id.toString()))  } },
        { new: true });
    })
}
...
function addEpisodes({ id, season_number, show_id, list }) {
  ...
  .then(data => {
    ...
    return Promise.all(episodes.map(episode => Media.findOneAndUpdate(
