    ...
    return (
      <QueryHandler query={listQuery} variables={{ id: this.props.match.params.id }}>
        {({ data, loading, error, client }) => {
          ...
          if (!isOwner) return <p style={{ color: 'red' }}>Error: Unauthorized</p>;
          return (
            <main>
              <div className="subheader">
                <h2>Editing {data.list.name}</h2>
                <button className="right" onClick={() => this.props.history.push(`/lists/${this.props.match.params.id}`)}>
                  RETURN TO LIST
                </button>
              ...
              <form className="search">
                ...
                >
                  {(editList, { loading, error }) => (
                    <button onClick={e => {
                      e.preventDefault();
                      editList({ variables: { id: this.props.match.params.id, name: this.state.text }})
                    }}>
                      SUBMIT
                    </button>
                  )}
              ...
              >
                {(deleteList, { loading, error }) => (
                  <button className="standalone-btn" onClick={() => deleteList({ variables: { id: this.props.match.params.id } })}>
                    DELETE
                  </button>
                )}
              </Mutation>
